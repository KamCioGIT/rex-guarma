...
RegisterNetEvent('rex-guarma:client:buyticket', function()
    local input = lib.inputDialog(Lang:t('client.lang_1'), {
        { 
            label = Lang:t('client.lang_2'),
            description = Lang:t('client.lang_3')..Config.TicketCost,
            type = 'input',
            min = 1,
            max = 10,
            required = true,
...
AddEventHandler('rex-guarma:client:dotravel', function(data)
    ...
    if hasItem then
        if data.destination == 'guarma' then
            TriggerServerEvent('rex-guarma:server:removeItem', 'boatticket', 1)
            DoScreenFadeOut(500)
            Wait(1000)
            ...
            Wait(10000)
            Citizen.InvokeNative(0x1E5B70E53DB661E5, 0, 0, 0, Lang:t('client.lang_4'), Lang:t('client.lang_5'), Lang:t('client.lang_6'))
            Wait(20000)
        ...
        if data.destination == 'stdenis' then
            ...
            DoScreenFadeOut(500)
            Wait(1000)
            Citizen.InvokeNative(0x1E5B70E53DB661E5, 0, 0, 0, Lang:t('client.lang_7'), Lang:t('client.lang_8'), Lang:t('client.lang_9'))
            Wait(30000)
        ...
        lib.notify({ title = Lang:t('client.lang_10'), duration = 5000, type = 'error' })
    end
end)
